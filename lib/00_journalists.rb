...
def menu
  ...
  while user_input < 1 || user_input > 9
    ...
    puts "Veuillez entrer le chiffre (1-9) de l'action souhaitée"
   end
   user_input
  end


def count_journalists (data)  
  puts "Le segment de data contient #{data.count} journalistes"
  puts "Pressez Enter pour continuer.."
  gets
end

def shortest (data) 
  puts "Voici le handle le plus court du segment de data"
  puts data.sort_by(&:length)[0]
  puts "Pressez Enter pour continuer.."
  gets
end

def count_handle_leght_5 (data) 
  puts "Le segment de data contient #{data.select{|handle| handle.length == 6}.length} handles contenant 5 caractères (hors @)"
  puts "Pressez Enter pour continuer.."
  gets
end

def count_start_cap (data)  
...
def epenser_index (data)  
  puts "Le handle @epenser si situe à l'index : #{data.index("@epenser")}"
  puts "Pressez Enter pour continuer.."
  gets
end

def length_distribution (data)  #8 Sors-moi une répartition des handle par taille de ces derniers (nombre de handle avec 1 caractère, nombre de handle avec 2 caractères, nombre de handle avec 3 caractères, etc)
  puts "Le segment de data contient"
  handle_max = data.sort_by(&:length).last.length
      for i in (2..handle_max)
    puts "#{data.select{ |handle| handle.length == i}.length} handles avec #{i-1} caractères (hors @)"
    end  
  puts "Pressez Enter pour continuer.."
  gets
end
